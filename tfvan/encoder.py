...
def Van(embed_dims, mlp_ratios, depths, drop_rate=0., path_drop=0.1, model_name='van',
        ...
        classifier_activation='softmax', include_skips=True):
    ...
    skips = [model.get_layer(name=f'norm{i+1}').output for i in range(len(depths)-1)]
    model = models.Model(inputs=inputs, outputs=[outputs] + skips[::-1], name=model_name)
    return model


def VanTinyEncoder(model_name='van_tiny', embed_dims=(32, 64, 160, 256), mlp_ratios=(8, 8, 4, 4), depths=(3, 3, 5, 2),
            weights='imagenet', include_skips=True, include_top=False, **kwargs):
    return Van(model_name=model_name, embed_dims=embed_dims, mlp_ratios=mlp_ratios,
               depths=depths, weights=weights, include_skips=include_skips, include_top=include_top, **kwargs)
